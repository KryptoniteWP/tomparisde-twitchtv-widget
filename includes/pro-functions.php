...
function tp_twitch_pre_pro_the_widget_upgrade_info() {
    ...
    <p>
        ...
        // translators: %s: URL to the Pro version
        printf( wp_kses( __( 'You want more features? <a href="%s" target="_blank" rel="nofollow">Check out the Pro version</a>!', 'tomparisde-twitchtv-widget' ), array(  'a' => array( 'href' => array(), 'target' => '_blank', 'rel' => 'nofollow' ) ) ), esc_url( tp_twitch_get_pro_version_url( 'widgets-page', 'textlink' ) ) ); ?>
    </p>
    <?php
}
...
function tp_twitch_pre_pro_the_widget_streams_max_note() {
    ...
    echo '<small>';
    // translators: %d: Maximum number of streams
    printf( 
        // translators: %d: Maximum number of streams that can be displayed
        esc_html__( 'You can display a maximum of %d streams.', 'tomparisde-twitchtv-widget' ), 
        esc_html( tp_twitch_get_default_streams_max() )
    );
    echo '<br />';
    // translators: %s: URL to upgrade
    printf( 
        wp_kses( 
            // translators: %s: URL to upgrade
            __( '<a href="%s" target="_blank">Upgrade now</a> in order to show more streams.', 'tomparisde-twitchtv-widget' ),
            array( 'a' => array( 'href' => array(), 'target' => array( '_blank' ) ) )
        ), 
        esc_url( tp_twitch_get_pro_version_url( 'widgets-page', 'streams-max-note' ) )
    );
    echo '</small>';
}
